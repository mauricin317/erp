...
async function eliminarCategoria(idcategoria) {
   ...
   var standardConfig =  { id : 'idcategoria', parentid : 'idcategoriapadre'};
...
export default function CategoriasTreeView(){
    ...
    const [state, setState] = useState({
      ...
      const handleEliminar = async () => {
        let node = state.categorias[0].getNodeById(selected)
        let eliminar = await eliminarCategoria(node.dataObj.idcategoria);
        if(eliminar.ok){
          handleSubmit();
        }else{
      ...
      const handleSelect = (event, nodeId) => {
        ...
        if(nodeId === "0"){
          ...
          setDisabled({new: false,edit: false, delete: node.children.length > 0})
        }
      };
      ...
      const renderTree = (node) => (
        <CustomTreeItem key={node.id} nodeId={node.id+""} label={node.dataObj.nombre}>
    ...
    return(
        ...
        <Button sx={{m:1}} variant="outlined" size="small" color="secondary" disabled={state.categorias.length<=0} onClick={handleExpandClick}>
        ...
        <AlertDialog open={openDialog} title={"¿Seguro que desea eliminar esta Categoría?"} body={"La categoría se eliminará de forma permanente"} btnText={"Eliminar"} close={() => setOpenDialog(false)} confirm={handleEliminar} />
