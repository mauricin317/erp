...
export default function FormNotaVenta(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (detalles.length >= 1) {
        ...
        let resultado = await crearNotaVenta(notaventa, props.jwt);
        if (resultado.ok) {
  ...
  const cargarDetalles = async () => {
    let traerDetalles = await obtenerDetallesNotaVenta(
      formData.idnota,
      props.jwt
    );
    if (traerDetalles.ok) {
  ...
  let _articulos = _.map(formData.articulos, (articulo) => {
    let _articulo = _.cloneDeep(articulo);
    let _detalles = _.map(
      _.filter(detalles, ["idarticulo", articulo.idarticulo]),
      "nrolote"
    );
  ...
  const handleAnular = async () => {
    ...
    let respuesta = await anularNotaVenta(data, props.jwt);
    if (respuesta.ok) {
      setEstado("Anulado");
  ...
  const openReport = () => {
    window.open(
      `http://localhost:8080/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit=%2Freports%2FNotaVenta&standAlone=true&id_nota=${formData.idnota}&j_username=joeuser&j_password=123&sessionDecorator=no`,
      "_blank"
    );
  };
  ...
  return (
    ...
    >
      ...
      <AlertDialog
        ...
        body={"Se perderán todos los datos de esta nota de venta"}
        btnText={"Sí, Salir"}
      ...
      <AlertDialog
        ...
        title={"¿Seguro que desea anular esta Nota de Venta?"}
        body={
