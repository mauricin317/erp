...
async function crearNotaVenta(datos) {
  return fetch('http://localhost:3000/api/notasventa', {
    method: 'POST',
    headers: {
...
async function obtenerDetalles(idnota) {
  return fetch(`http://localhost:3000/api/notasventa/detalles/${idnota}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
...
async function anularNotaVenta(datos) {
  return fetch(`http://localhost:3000/api/notasventa/`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
...
export default function FormNotaVenta(props){
      ...
      onSubmit: async values => {
        if(detalles.length >= 1){
          ...
          subtotales.forEach(elem => {
            ...
            let resultado = await crearNotaVenta(notaventa);
            if (resultado.ok){
              toast.success(resultado.mensaje,{theme: "colored"});
              props.enableReadOnly(notaventa, resultado.datos.idnota, resultado.datos.nronota);
            }else{
              toast.error(resultado.mensaje,{theme: "colored"});
            }
        }else{
          toast.error("Debe agregar 1 articulo como mínimo",{theme: "colored"});
        }
      }
    });
    ...
    const cargarDetalles = async () =>{
      let traerDetalles = await obtenerDetalles(formData.idnota)
      if(traerDetalles.ok){
    ...
    //let _articulos = _.differenceBy(props.formData.articulos,detalles, 'idarticulo');
    let _articulos = _.map(props.formData.articulos, (articulo) =>{
      let _articulo = _.cloneDeep(articulo)
      let _detalles = _.map(_.filter(detalles, ['idarticulo', articulo.idarticulo]), 'nrolote');
      let lotes = _articulo.lotes
      _articulo.lotes = _.differenceBy(lotes, _detalles);
      return _articulo;
    })
    _.remove(_articulos, function(n) {
      return (n.lotes).length == 0;
    });
    ...
    const handleEditarDetalle = (detalle) =>{
      setModalform({
        ...
        tipo:'editar',
        datos:detalle,
        articulo: props.formData.articulos
      });
    }
    ...
    const handleAnular = async () => {
      ...
      let respuesta = await anularNotaVenta(data);
      if (respuesta.ok){
        setEstado("Anulado");
    ...
    const openReport= () =>{
      window.open(`http://localhost:8080/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit=%2Freports%2FNotaVenta&standAlone=true&id_nota=${formData.idnota}&j_username=joeuser&j_password=123&sessionDecorator=no`, '_blank');
    }
    ...
    return(
        ...
        <form onSubmit={formik.handleSubmit}>   
            ...
            <h2>{`${!props.readOnly ? 'Nueva' : 'Ver'} Nota de Venta`}</h2>
          ...
          <AlertDialog open={openDialog.state} title={"¿Seguro que desea salir?"} body={"Se perderán todos los datos de esta nota de venta"} btnText={"Sí, Salir"} close={() => setOpenDialog({state:false})} confirm={props.closeForm} />
          <AlertDialog open={openDialogAnular.state} title={"¿Seguro que desea anular esta Nota de Venta?"} body={"La nota se anulará junto con su comprobante (Si tiene habilitada la integración)"} btnText={"Anular"} close={() => setOpenDialogAnular({state:false})} confirm={handleAnular} />
          <ModalForm open={modalform.open} tipo={modalform.tipo} datos={modalform.datos} close={handleCloseModal} articulos={modalform.tipo == "nuevo" ? _articulos : modalform.articulo} submit={handleSubmitDetalle}/>
    </Box>
    )
}
