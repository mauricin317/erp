...
import { Select, OutlinedInput, InputLabel, MenuItem, FormControl, Chip } from '@mui/material';
...
async function crearArticulo(datos) {
 ...
 async function editarArticulo(datos, idarticulo) {
...
export default function FormArticulo(props){
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
          ...
          precioventa: Yup.number().typeError('Debe ser un número').positive("Debe ser un número positivo").required('Requerido')
        }),
        onSubmit: async values => {
          if(props.tipo === "nuevo"){
            if(categorias.length <= 0){
              toast.error("Debe seleccionar una o más categorías",{theme:'colored'})
              return
            }
            let data={
            ...
            let crear = await crearArticulo(data);
            if(crear.ok){
          ...
          if(props.tipo === "editar"){
            ...
            let editar = await editarArticulo(data,articulo.idarticulo);
            if(editar.ok){
        ...
        <form onSubmit={formik.handleSubmit}>   
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={9} xs={12}>
                <Autocomplete sx={{ mt: '16px', mb: '8px', width: '100%' }}
                  multiple
                  id="tags-outlined"
                  ...
                  renderTags={(value, getTagProps) =>
                    value.map((option, index) => (
                      <Chip key={index} color='success' label={option.nombre} {...getTagProps({ index })} />
                    ))
                  }
                  renderInput={(params) => (
                    <TextField
