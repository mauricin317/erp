...
export default function CuentasTreeView(props) {
  ...
  const handleReport = () => {
    window.open(
      `http://localhost:8080/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit=%2Freports%2FPlanCuentas&standAlone=true&id_empresa=${state.idempresa}&j_username=joeuser&j_password=123&sessionDecorator=no`,
      "_blank"
    );
  };
  ...
  const renderTree = (node) => (
    <CustomTreeItem
      key={node.id}
      nodeId={node.id + ""}
      label={`${node.dataObj.codigo} - ${node.dataObj.nombre}`}
    >
  ...
  return (
    ...
    >
      ...
      <AlertDialog
        ...
        body={"La cuenta se eliminará de forma permanente"}
        btnText={"Eliminar"}
