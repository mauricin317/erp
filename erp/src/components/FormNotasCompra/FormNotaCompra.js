...
async function crearNotaCompra(datos) {
...
async function obtenerDetalles(idnota) {
  return fetch(`http://localhost:3000/api/notascompra/detalles/${idnota}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
...
async function anularNotaCompra(datos) {
  return fetch(`http://localhost:3000/api/notascompra/`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
...
export default function FormNotaCompra(props){
      ...
      onSubmit: async values => {
        if(detalles.length >= 1){
          ...
          subtotales.forEach(elem => {
            ...
            let resultado = await crearNotaCompra(notacompra);
            if (resultado.ok){
              toast.success(resultado.mensaje,{theme: "colored"});
              props.enableReadOnly(notacompra, resultado.datos.idnota, resultado.datos.nronota);
            }else{
    ...
    const handleEditarDetalle = (detalle) =>{
      ...
      setModalform({
        ...
        tipo:'editar',
        datos:detalle,
        articulo: _.differenceBy(props.formData.articulos,detalles_hack, 'idarticulo')
      });
    }
    ...
    const handleAnular = async () => {
      ...
      let respuesta = await anularNotaCompra(data);
      if (respuesta.ok){
        setEstado("Anulado");
    ...
    const openReport= () =>{
      window.open(`http://localhost:8080/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit=%2Freports%2FNotaCompra&standAlone=true&id_nota=${formData.idnota}&j_username=joeuser&j_password=123&sessionDecorator=no`, '_blank');
    }
    ...
    return(
        ...
        <form onSubmit={formik.handleSubmit}>   
          ...
          <AlertDialog open={openDialog.state} title={"¿Seguro que desea salir?"} body={"Se perderán todos los datos de esta nota de compra"} btnText={"Sí, Salir"} close={() => setOpenDialog({state:false})} confirm={props.closeForm} />
          <AlertDialog open={openDialogAnular.state} title={"¿Seguro que desea anular esta Nota de Compra?"} body={"La nota se anulará junto con su comprobante (Si tiene habilitada la integración)"} btnText={"Anular"} close={() => setOpenDialogAnular({state:false})} confirm={handleAnular} />
